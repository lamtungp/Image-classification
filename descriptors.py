...
def all_descriptors(dataset, class_list, option=constants.ORB_FEAT_OPTION):
    ...
    for i in range(len(class_list)):
        message = "*** Getting descriptors for class number {0} of {1} ***".format(
            i, len(class_list))
        print(message)
...
def img_to_vect(img_descs, cluster_model):
    ...
    clustered_desc = [cluster_model.predict(
        raw_words) for raw_words in img_descs]
    print("clustered_desc ==== ", clustered_desc)
    
    img_bow_hist = np.array(np.bincount(
        clustered_desc, minlength=cluster_model.n_clusters))
    return img_bow_hist
