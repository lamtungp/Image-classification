...
def test_dataset():
    ...
    pickle.dump(dataset, open(constants.DATASET_OBJ_FILENAME, "wb"),
...
def test_descriptors():
    img = cv2.imread(constants.TESTING_IMG_PATH)
    print(constants.TESTING_IMG_PATH)
...
def test_codebook():
    ...
    elapsed_time = utils.humanize_time(end - start)
    print("Elapsed time getting all the descriptors is {0}".format(
        elapsed_time))
    k = 64
    ...
    elapsed_time = utils.humanize_time(end - start)
    print("Elapsed time calculating the k means for the codebook is {0}".format(
        elapsed_time))
    np.savetxt(codebook_filename, codebook,
               delimiter=constants.NUMPY_DELIMITER)
    print("Codebook loaded in {0}, press any key to exit ...".format(
...
def test_vlad():
    ...
    codebook_filename = "codebook_{0}_{1}.csv".format(k, des_name)
    centers = np.loadtxt(
        codebook_filename, delimiter=constants.NUMPY_DELIMITER)
